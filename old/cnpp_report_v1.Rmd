...
Armed with learnings from the previous versions of the program, the City launched the Commuters Trust Community Nonprofit Partner Program (CNPP) in October 2021. The program provides free transportation benefits in the form of bus passes and rideshare rides to workers working with 13 social service providers in South Bend. These organizations provide a variety of housing, education, job training, healthcare, and other essential services aimed at alleviating poverty or disability. The program is being jointly funded by the City and nonprofit partners, and is expected to continue through 2022.

The CNPP program complements the work-travel subsidy program and reduces dependence on non-profits. The CNPP program targets a population much different from those in the work-travel subsidy program. These programs also differ in their intended use. While in the CNPP program, the ride locations are unrestricted. In the work-travel subsidy program, the rides are intended to be used to travel to work. Taken together, both programs complement each other by targeting different populations and intended use. Additionally, the CNPP program complements the services of non-profit partners helping them focus on other issues.
...
uber_fig <- Uber_CNPP %>%
  ...
  labs(caption = paste0("Average rides per rider is shown at base of the bars","\n",
         "Total rides = ",nrow(Uber_CNPP),
         "Total unique riders=",n_distinct(Uber_CNPP$`Full Name`),"\n"))

Buspass_data_edit <- read_excel("data/ride data/Transpo_0822.xlsx", 
                                sheet = "August Rides")
...
6. The program saves money, reduces stress, makes users feel independent, get a job, complete education
...
7. The program has spillover effects on broader community impacts
...
8. The program may not solve all the problems, but it does go a long way
...
9. The city and nonprofits have done a tremendous job of distributing the bus pass
...
The program may not solve all the problems, but it does go a long way...

<Recommendations here>

## Minor issues {-}
1.	One person mentioned that Uber vouchers are difficult to use because he has to first use the payment method as a checking account. Then switch to voucher once the ride is dispatched. Though this experience was not shared by all users.
...
3.	How to use Uber app/voucher training: Some participants mentioned that they would like to try out using the Uber app but were unsure how to learn to do so. The city can create a training video to show how to use the app and monthly voucher.
